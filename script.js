...
function initAdminPanel() {
    ...
    adminNavButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            const contentId = this.textContent.toLowerCase().replace(' ', '-');
    ...
    if (contentForm) {
        contentForm.addEventListener('submit', function(e) {
            ...
            if (contentAction === 'add') {
                ...
                // Log activity
                logUserActivity(currentUser.id, 'Add Content', `Added ${contentType}: ${title}`);
                
                showNotification(`${contentType} added successfully!`, 'success');
            } else {
                ...
                if (contentIndex !== -1) {
                    contentItems[contentIndex].title = title;
                    contentItems[contentIndex].description = description;
                    contentItems[contentIndex].category = category;
                    contentItems[contentIndex].status = status;
                    
                    // Log activity
                    logUserActivity(currentUser.id, 'Edit Content', `Edited ${contentType}: ${title}`);
                    
                    showNotification(`${contentType} updated successfully!`, 'success');
                }
            }
...
function editUser(userId) {
    ...
    // Create edit form modal (simplified for demo)
            ...
            <div class="modal-body">
                <form id="edit-user-form">
                    ...
                    <div class="form-actions">
                        <button type="button" class="btn-primary" onclick="saveUserEdit(${user.id})">Save</button>
...
function saveUserEdit(userId) {
    ...
    // Log activity
    logUserActivity(currentUser.id, 'Edit User', `Edited user: ${name}`);
...
function deleteUser(userId) {
    ...
    if (index !== -1) {
        ...
        // Log activity
        logUserActivity(currentUser.id, 'Delete User', `Deleted user: ${user.name}`);
